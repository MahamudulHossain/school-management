...
class SettingController extends Controller
{
    ...
    {
       abort_if(Gate::denies('SettingsAccess'), redirect('error'));
        $settings = Setting::first();
        $countries = DB::table('countries')->orderBy('country_title', 'asc')->get();
//        dd($settings);
        return view('settings.index', compact('settings', 'countries'));
    }
...
//        dd(Response::HTTP_FORBIDDEN);
    ...
    {
        if ($code == 'Ei$2021') {
            $filename = "lara12bp-" . date('YmdHis') . ".sql";
