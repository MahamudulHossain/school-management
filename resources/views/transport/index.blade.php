...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Transport</a>
    </li>
@endsection
@push('css')
            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">Transportation</a>
                </li>
                <li class="nav-item">
                        <a href="{{ url('transport/create') }}" class="nav-link">
                        Add Transport Info
                    </a>
                </li>
            </ul>
                ...
                <tbody>
                    @foreach($transports as $key => $transport)
                        <tr>
                            ...
                            <td>{{$transport->route_info}}</td>
                            <td>{{$transport->vehicle_info}}</td>
                            <td>{{$transport->driver_info }}</td>
                            <td>{{$transport->fare }}</td>
                            <td>
                                <a href="{{ url('transport/' . $transport->id . '/edit') }}" class="btn btn-info btn-sm" title="Edit"><span class="far fa-edit" aria-hidden="true"></span></a>
                                <form method="POST" action="{{ url('transport/' . $transport->id) }}" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
            ...
            buttons: [
...

                {
                    ...
                    customize: function (doc) {
                        ...
                        doc.content[1].table.widths = ['15%', '15%', '15%', '15%', '15%', '25%'];
