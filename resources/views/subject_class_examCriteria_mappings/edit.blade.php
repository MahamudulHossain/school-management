...
@section('title','Edit Subject Class Exam Criteria Mapping')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{url('subject-class-exam-criteria')}}" class="nav-link">Subject Class Exam Criteria Mapping</a>
    </li>
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0);" class="nav-link">Edit Subject Class Exam Criteria Mapping</a>
    </li>
@endsection
@push('css')
        ...
        <div class="col-md-8">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Edit Subject Class Exam Criteria Mapping</h3>
                </div>
                <form id="saveForm" method="POST" action="{{ url('subject-class-exam-criteria/'.$school_class_id.'/'.$subject_id) }}" class="form-horizontal">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                        ...
                        <div class="col-md-6">
                            <select name="subject_id" class="form-control" id="subject_id" required>
                                ...
                                @foreach($subjects as $id => $subject)
                                    <option value="{{$subject->subject_id}}" {{ $subject->subject_id == $subject_id ? 'selected' : '' }}>{{$subject->subject->subject_name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            <select name="exam_criteria_id[]" class="form-control select2" multiple="multiple" required>
                                @foreach($examCriterias as $criteria)
                                    <option value="{{$criteria->id}}" {{ in_array($criteria->id, $mapping) ? 'selected' : '' }}>{{$criteria->criteria_name}}</option>
                                @endforeach
                            </select>
