...
@section('title','Subject Class Exam Criteria Mapping')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Subject Class Exam Criteria Mapping</a>
    </li>
@endsection
@push('css')
    ...
    <div class="card card-success card-tabs">
            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">Subject Class Exam Criteria Mapping</a>
                </li>
                <li class="nav-item">
                        <a href="{{ url('subject-class-exam-criteria/create') }}" class="nav-link">
                        Add Subject Class Exam Criteria Mapping
                    </a>
                </li>
        ...
        <div class="card-body">
            <div class="tab-content" id="custom-tabs-one-tabContent">
                ...
                <tbody>
                    @foreach($flatArr as $key => $datum)
                    {{-- @dd($datum) --}}
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $datum->school_class->class_name }}</td>
                        <td>{{ $datum->subject->subject_name }}</td>
                        <td>
                            @foreach($datum->exam_criteria_names as $criteria)
                                {{ $criteria }}@if(!$loop->last), @endif
                            @endforeach
                        </td>
                        <td class="noprint">
                            <a href="{{ url('subject-class-exam-criteria/'.$datum->school_class->id.'/'.$datum->subject->id.'/edit') }}" class="btn btn-sm btn-primary" title="Edit">
                                <i class="fa fa-edit"></i>
                            </a>
                            <form method="get" action="{{ url('subject-class-exam-criteria/' . $datum->school_class->id . '/' . $datum->subject->id) }}" style="display:inline">
                                @csrf
                                <button type="submit"
        ...
        $('.dataTables').DataTable({
            ...
            buttons: [
                ...
                {extend: 'csv'},
                {
                    ...
                    messageTop: ' Subject Class Exam Criteria Mapping   '
                },
...

                {
                    ...
                    filename: 'Subject Class Exam Criteria Mapping ',
                    extension: '.pdf',
                    orientation: 'portrait',
                    title: "Subject Class Exam Criteria Mapping ",
                    footer: true,
                    exportOptions: {
                        columns: ':not(.noprint)',
                    ...
                    customize: function (doc) {
                        ...
                        doc.content[1].table.widths = ['10%', '15%', '15%', '60%'];
