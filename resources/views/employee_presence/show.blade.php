...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="#" class="nav-link">View Employee Attendance</a>
    </li>
@endsection
...
@section('maincontent')
    <div class="card card-success card-tabs">
            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">View Employee Attendance</a>
                </li>
            ...
            <table class="table dataTables table-striped table-bordered table-hover">
                ...
                @foreach ($date_presence as $key => $dp)
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>
                            {{$dp->user->employee->first_name.' '.$dp->user->employee->middle_name.' '.$dp->user->employee->last_name}}
                            ({{ $dp->user->personnel_id }})
                        </td>
                        <td>{{ Carbon\Carbon::parse($dp->date)->format('d-M-Y') ?? '' }}</td>
                        <td>{{ $dp->in_time ? Carbon\Carbon::parse($dp->in_time)->format('h:i a') : '' }}</td>
                        <td>{{ $dp->out_time ? Carbon\Carbon::parse($dp->out_time)->format('h:i a') : ''}}</td>
                        <td>{{ $dp->remarks }}</td>
                    </tr>
                @endforeach
            </table>
        ...
        <div class="card-footer d-flex justify-content-end">
            <a href="{{ url('employee_presence/' . $date. '/edit' ) }}" class="btn btn-info" role="button">Update Attendance</a>
        </div>
    </div>
@endsection
@push('js')
...
//                {targets: [4], className: 'text-right'},
            ...
            buttons: [
                ...
                {extend: 'csv'},
                {
                    ...
                    messageTop: ' Employee Attendance   '
                },
