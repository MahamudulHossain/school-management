    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card card-primary">
                ...
                <form action="{{ route("attendance.store") }}" method="POST" id="saveForm">
                    @csrf
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                @foreach($student as $stu)
                                    <tr>
                                        <td>{{ $stu->user->personnel_id }}</td>
                                        <td>{{ $stu->user->student->first_name }} {{ $stu->user->student->last_name }}
                                            <input type="hidden" name="student_id[]" value="{{ $stu->user->student->id }}">
                                        </td>
                                        <td>{{$stu->roll}}
