            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">Exam Report of {{ isset($exams[0]) ? $exams[0]->examtype_name : '' }}</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="custom-tabs-one-tabContent">
                <div class="tab-pane fade show active" id="custom-tabs-one-home" role="tabpanel"
                    ...
                    <div class="row mb-3" style="display: {{ count($exams) > 0 ? '' : 'none' }}">
                        <div class="col-md-6">
                            <div class="p-3 border rounded bg-light shadow-sm text-center" style="font-size: 18px;">
                                <span class="fw-bold text-primary d-block">Highest Mark</span>
                                <span class="badge bg-success px-3 py-2 fs-6">{{ $maxMarks }}</span>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="p-3 border rounded bg-light shadow-sm text-center" style="font-size: 18px;">
                                <span class="fw-bold text-primary d-block">Average Mark</span>
                                <span class="badge bg-info text-light px-3 py-2 fs-6">{{ number_format($avgMarks, 2) }}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach($exams as $key => $exam)
                                <tr>
                                    ...
                                    <td>
                                        {{ $exam->user->student->first_name ?? '' }} {{ $exam->user->student->middle_name ?? '' }} {{ $exam->user->student->last_name ?? '' }}
                                    </td>
                                    <td>
                                        {{ $exam->roll ?? '' }}
                                    </td>
