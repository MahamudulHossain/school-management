...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ url('exam-marks-check') }}" class="nav-link">Exam Marks Check</a>
    </li>
@endsection
@push('css')
            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">Exam Marks Check</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="custom-tabs-one-tabContent">
                <div class="tab-pane fade show active" id="custom-tabs-one-home" role="tabpanel"
                    ...
                    <table class="table dataTables table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($mcheck as $key => $examtype)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $examtype->examtype_name }}</td>
                                    <td>{{ $examtype->full_marks }}</td>
                                    <td>{{ $examtype->pass_marks }}</td>
                                    <td style="color: {{ ($examtype->exam->count()>0) ? 'green' : 'red' }}">{{ ($examtype->exam->count()>0) ? 'Marks Inputed' : 'Marks Not Inputed' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
//            dom: '<"html5buttons"B>lTfgtip',
            ...
            buttons: [
...

                {
                    ...
                    filename: 'Exam Marks Check ',
                    extension: '.pdf',
//                    orientation : 'landscape',
                    orientation: 'portrait',
                    title: "Exam Marks Check ",
                    footer: true,
                    exportOptions: {
                    ...
                    customize: function (doc) {
                        ...
                        doc.content[1].table.widths = ['10%', '40%', '10%', '10%', '30%'];
