                    ...
                    <table class="table dataTables table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($routines as $index => $routine)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $routine->student_class->class_name ?? 'N/A' }}</td>
                                    <td>{{ $routine->student_section->section_name ?? 'N/A' }}</td>
                                    <td>{{ $routine->shift == '0' ? 'Morning' : 'Day' }}</td>
                                    <td>{{ $routine->academic_year->title ?? 'N/A' }}</td>
                                    <td>
                                        <a href="{{ route('class-routine.view', [
                                            'school_class_id' => $routine->school_class_id,
                                            'school_section_id' => $routine->school_section_id,
                                            'academic_year_id' => $routine->academic_year_id,
                                            'shift' => $routine->shift
                                        ]) }}" class="btn btn-sm btn-info text-white"><span class="far fa-eye" aria-hidden="true"></span></a>
                                        @can('ClassRoutineAccess')
                                            <a href="{{ route('class-routine.create', [
                                                'school_class_id' => $routine->school_class_id,
                                                'school_section_id' => $routine->school_section_id,
                                                'academic_year_id' => $routine->academic_year_id,
                                                'shift' => $routine->shift
                                            ]) }}" class="btn btn-sm btn-primary"><span class="far fa-edit" aria-hidden="true"></span></a>
                                        @endcan
                                        @can('ClassRoutineAccess')
                                            <form action="{{ route('class-routine.destroy', [
                                                'school_class_id' => $routine->school_class_id,
                                                'school_section_id' => $routine->school_section_id,
                                                'academic_year_id' => $routine->academic_year_id,
                                                'shift' => $routine->shift
