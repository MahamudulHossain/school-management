...
@section('manage_teacher','active')
@section('title','Manage Teacher')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Teacher</a>
    </li>
@endsection
@push('css')
<style>
                ...
                <tbody>
                    @foreach($teachers as $key => $teacher)
                        <tr>
                            ...
                            <td>{{ $teacher->email }}</td>
                            <td>
                                {{ $teacher->teacher->first_name.' '.$teacher->teacher->middle_name.' '.$teacher->teacher->last_name }}
                                <br>
                                {{ $teacher->personnel_id }}
                            </td>
                            <td>{{ $teacher->user_type->title }}</td>
                            <td>
                                @if(!empty($teacher->roles))
                                    @foreach($teacher->roles as $v)
                                        <span class="badge text-bg-primary">{{ $v->title }}</span>
                                    @endforeach
                                @endif
                            </td>
                            <td>
                                <a href="{{ url('user/' . $teacher->id) }}" class="btn btn-success btn-sm" title="View"><span class="far fa-eye" aria-hidden="true"></span></a>
                                {{--@can('role-edit')--}}
                                    <a href="{{ url('user/' . $teacher->id . '/edit') }}" class="btn btn-info btn-sm" title="Edit"><span class="far fa-edit" aria-hidden="true"></span></a>
                                {{--@endcan--}}
                                {{--@can('role-delete')--}}
                                    <form method="POST" action="{{ url('user/' . $teacher->id) }}" style="display:inline">
                                        @csrf
                                        @method('DELETE')
