...
@section('title','Edit Exam Criteria')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{url('exam-criteria')}}" class="nav-link">Manage Exam Criteria</a>
    </li>
    <li class="nav-item d-none d-sm-inline-block">
        <a href="#" class="nav-link">Edit Exam Criteria</a>
    </li>
@endsection
@push('css')
@endpush
        ...
        <div class="col-md-8">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Edit Exam Criteria</h3>
                </div>
                <form action="{{ url('exam-criteria/'.$examCriteria->id) }}" method="POST" id="saveForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('criteria_name') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input type="text" id="criteria_name" name="criteria_name" class="form-control" autofocus
                                    value="{{ $examCriteria->criteria_name }}" required>
                            ...
                            <div class="col-md-6 mt-radio-inline">
                                <label class="mt-radio">
                                    <input type="radio" name="status"
                                        value="active" {{ $examCriteria->status === 'active' ? 'checked' : '' }}>Active
                                    <span></span>
                                </label>
                                <label class="mt-radio">
                                    <input type="radio" name="status"
                                        value="inactive" {{ $examCriteria->status === 'inactive' ? 'checked' : '' }}>Inactive
                                    <span></span>
                                </label>
                            </div>
                        </div>
