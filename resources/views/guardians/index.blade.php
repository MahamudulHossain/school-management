...
@section('guardian_mo','menu-open')
@section('guardian','active')
@section('manage_guardian','active')
@section('title','Manage Guardian')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Guardian</a>
    </li>
@endsection
@push('css')
<style>
                ...
                <tbody>
                    @foreach($guardians as $key => $guardian)
                        <tr>
                            ...
                            <td>{{ $guardian->cell_phone }}</td>
                            <td>
                                {{ $guardian->personnel_id }}
                            </td>
                            <td>{{ $guardian->user_type->title }}</td>
                            <td>
                                @if(!empty($guardian->roles))
                                    @foreach($guardian->roles as $v)
                                        <span class="badge text-bg-primary">{{ $v->title }}</span>
                                    @endforeach
                                @endif
                            </td>
                            <td>
                                <a href="{{ url('user/' . $guardian->id) }}" class="btn btn-success btn-sm" title="View"><span class="far fa-eye" aria-hidden="true"></span></a>
                                {{--@can('role-edit')--}}
                                <a href="{{ url('user/' . $guardian->id . '/edit') }}" class="btn btn-info btn-sm" title="Edit"><span class="far fa-edit" aria-hidden="true"></span></a>
                                {{--@endcan--}}
                                @can('GuardianAccess')
                                    <form method="POST" action="{{ url('user/' . $guardian->id) }}" style="display:inline">
                                        @csrf
                                        @method('DELETE')
