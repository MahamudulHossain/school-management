...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Employee</a>
    </li>
@endsection
@push('css')
<style>
                ...
                <tbody>
                    @foreach($employees as $key => $employee)
                        <tr>
                            ...
                            <td>{{ $employee->user_type->title }}</td>
                            <td>
                                @if(!empty($employee->roles))
                                    @foreach($employee->roles as $v)
                                        <span class="badge text-bg-primary">{{ $v->title }}</span>
                                    @endforeach
                                @endif
                            </td>
                            <td>
                                <a href="{{ url('user/' . $employee->id) }}" class="btn btn-success btn-sm" title="View"><span class="far fa-eye" aria-hidden="true"></span></a>
                                {{--@can('role-edit')--}}
                                    <a href="{{ url('user/' . $employee->id . '/edit') }}" class="btn btn-info btn-sm" title="Edit"><span class="far fa-edit" aria-hidden="true"></span></a>
                                {{--@endcan--}}
                                @can('EmployeeDelete')
                                    <form method="POST" action="{{ url('user/' . $employee->id) }}" style="display:inline">
                                        @csrf
                                        @method('DELETE')
