...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Shift</a>
    </li>
@endsection
@push('css')
            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">{{ $title }} Info</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
                    ...
                    <table class="table dataTables table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($data as $datum)
                                <tr>
                                    <td>{{ $datum->title }}</td>
                                    <td>{{ Carbon\Carbon::parse($datum->start_time)->format('h:i a') }}</td>
                                    <td>{{ Carbon\Carbon::parse($datum->end_time)->format('h:i a') }}</td>
                                    <td>
                                        <form method="POST" action="{{ url('shift/' . $datum->id) }}" style="display:inline">
                                            @csrf
                                            @method('DELETE')
