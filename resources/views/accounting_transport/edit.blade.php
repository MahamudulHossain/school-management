...
@section('title','Manage Transport fare')
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Transport fare</a>
    </li>
@endsection
@push('css')
        ...
        <div class="col-md-8">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Manage Transport fare</h3>
                </div>
                <form action="{{ route("accounting-transport.update",$data->id) }}" method="POST" id="saveForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('fee_title') ? 'has-error' : '' }}">
                            <label for="fee_title" class="col-md-4 control-label text-md-right">
                                Transport Info :<span class="required">*</span>
                            </label>
                            <div class="col-md-6">
                                <input type="text" id="fee_title" name="fee_title" value="{{ $data->transport->title }}" class="form-control" disabled>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('fare_amount') ? 'has-error' : '' }}">
                            <label for="fare_amount" class="col-md-4 control-label text-md-right">
                                Fare Amount :<span class="required">*</span>
                            </label>
                            <div class="col-md-6">
                                <input type="number" id="fare_amount" name="fare_amount" value="{{ $data->fare_amount }}" class="form-control" disabled>
                            </div>
                        </div>
