...
@section('breadcrumb')
    ...
    <li class="nav-item d-none d-sm-inline-block">
        <a href="#" class="nav-link">Update Exam Type</a>
    </li>
@endsection
@push('css')
@endpush
        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Update Exam Type</h3>
                </div>
                <form action="{{ route('exam-type.update', $examType->id) }}" method="POST" id="saveForm">
                    @method('PATCH')
                    @csrf
                    <div class="card-body">
                            ...
                            <div class=" col-md-6">
                                <select name="term_id" id="term_id" class="form-control" required>
                                    ...
                                    @foreach($terms as $id => $name)
                                        <option value="{{ $id }}" {{ $examType->term_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class=" col-md-6">
                                <select name="school_class_id" id="school_class_id" class="form-control" required>
                                    ...
                                    @foreach($schoolClasses as $id => $name)
                                        <option value="{{ $id }}" {{ $examType->school_class_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class=" col-md-6">
                                <select name="subject_id" id="subject_id" class="form-control" required>
                                    ...
                                    @foreach($subjects as $subject)
                                        <option value="{{ $subject->id }}" {{ $examType->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->subject_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class='row'>
                            <table class="table table-bordered table-hover ">
                                ...
                                <tbody>
                                    <?php $total_marks=0; ?>
                                    @foreach($etype as $stu)
                                        <?php $total_marks += $stu->full_marks; ?>
                                        <tr>
                                            <td>
                                                <input type="text" data-type="title"
                                                    id="examtype_name_<?php echo $stu->id?>"
                                                    value="{{ $stu->exam_criteria->criteria_name }}"
                                                    custom="doup" class="form-control " autocomplete="off" required readonly>
                                                <input type="hidden" name="examtype_name[]" value="{{ $stu->exam_criteria_id }}">
                                            </td>
                                            <td><input type="number" name="full_marks[]" id="full_marks_<?php echo $stu->id?>"
                                                    value="<?php echo $stu->full_marks?>"
                                                    class="form-control changesNo totalLinePrice" autocomplete="off" style="text-align:right"
                                                    onkeypress="return IsNumeric(event);" ondrop="return false;"
                                                    onpaste="return false;" required></td>
                                            <input type="hidden" data-type="id" name="id[]"
                                                                    id="id_<?php echo $stu->id?>"
                                                                    value="<?php echo $stu->id ?>"
                                                                    class="form-control" autocomplete="off">
                                            <td><input type="number" name="pass_marks[]" id="pass_marks_<?php echo $stu->id?>"
                                                    value="<?php echo $stu->pass_marks ?>"
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        ...
                                        <input readonly type="number" name="total" step="any" class="form-control" id="subTotal"
                                            ...
                                            onkeypress="return IsNumeric(event);" ondrop="return false;"
                                            onpaste="return false;" value="<?php echo $total_marks; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-footer d-flex justify-content-end">
                        ...
                        <button type="submit" class="btn btn-success float-right" style="margin-left: 5px" id="saveButton"><i
                                    class="fa fa-save"
