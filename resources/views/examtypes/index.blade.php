            ...
            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill"
                       ...
                       aria-selected="true">Manage Exam Type</a>
                </li>
                <li class="nav-item">
                        <a href="{{ url('exam-type/create') }}" class="nav-link">
                        Add Exam Type
                    </a>
                </li>
            </ul>
                    ...
                    <table class="table dataTables table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($examtypes as $key => $examtype)
                                <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $examtype->examtype_name }}</td>
                                        <td>{{ $examtype->full_marks }}</td>
                                        <td>{{ $examtype->pass_marks }}</td>

                                        <td class="noprint">
                                                <a href="{{ url('exam-type/' . $examtype->id . '/edit') }}" class="btn btn-primary btn-sm" title="Edit examtype"><i class="fa fa-edit" aria-hidden="true"></i></a>

                                                <form method="POST" action="{{ url('exam-type', $examtype->id) }}" accept-charset="UTF-8" style="display:inline">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button type="submit" class="btn btn-danger btn-sm" title="Delete examtype" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                                </form>
                                        </td>
                                    </tr>
                            @endforeach
