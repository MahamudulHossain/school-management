...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Update Expense</a>
    </li>
@endsection
@push('css')
    ...
    <div class="row justify-content-center ">
        <div class="col-md-8">
            <div class="card card-primary">
                ...
                <form action="{{ route('expense.update', $expense->id) }}" method="POST" id="saveForm">
                    @method('PATCH')
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <select name="expense_type_id" id="expense_type_id" class="form-control" required>
                                    ...
                                    @foreach ($expenseTypes as $expenseType)
                                        <option value="{{ $expenseType->id }}" {{ $expenseType->id == $expense->expense_type_id ? 'selected' : '' }}>{{ $expenseType->expense_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('comments') ? 'has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea name="comments" id="comments" cols="20" rows="5" class="form-control">{{ $expense->comments ?? '' }}</textarea>
