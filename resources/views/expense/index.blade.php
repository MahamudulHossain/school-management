...
@section('breadcrumb')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="javascript:void(0)" class="nav-link">Manage Expense</a>
    </li>
@endsection
@push('css')
                ...
                <tbody>
                    @foreach($expenses as $key => $expense)
                        <tr>
                            ...
                            <td>{{ $expense->expense_type->expense_name }}</td>
                            <td>{{ $expense->expense_amount }}</td>
                            <td>{{ $expense->comments ?? '' }}</td>
                            <td>
                                <a href="{{ url('expense/' . $expense->id . '/edit') }}" class="btn btn-info btn-sm" title="Edit"><span class="far fa-edit" aria-hidden="true"></span></a>
                                <form method="POST" action="{{ url('expense/' . $expense->id) }}" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
